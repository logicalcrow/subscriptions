...
class PlanSubscriptionFeature extends Model
{
    ...
    public function __construct(array $attributes = [])
    {
        ...
        $this->setTable(config('logicalcrow.tables.plan_subscription_features'));
    }
    ...
    public function getRules(): array
    {
        return [
            'tag' => [
                'required',
                'max:150',
                Rule::unique(config('logicalcrow.tables.plan_subscription_features'))->where(function ($query) {
                    return $query->where('id', '!=', $this->id)->where('plan_subscription_id', $this->plan_subscription_id);
                }),
            ],
            'plan_subscription_id' => 'required|integer|exists:' . config('logicalcrow.tables.plan_subscriptions') . ',id',
    ...
    {
        return $this->belongsTo(config('logicalcrow.models.plan_subscription'), 'plan_subscription_id', 'id');
    }
    ...
    {
        return $this->belongsTo(config('logicalcrow.models.plan_feature'), 'plan_feature_id', 'id');
    }
    ...
    {
        return $this->hasOne(config('logicalcrow.models.plan_subscription_usage'), 'plan_subscription_feature_id', 'id');
    }
