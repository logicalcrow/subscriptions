...
use Logicalcrow\Subscriptions\Exceptions\UsageDenied;
use Logicalcrow\Subscriptions\Services\Period;
use Logicalcrow\Subscriptions\Traits\BelongsToPlan;
use Logicalcrow\Subscriptions\Traits\HasFeatures;
use Logicalcrow\Subscriptions\Traits\HasGracePeriod;
use Logicalcrow\Subscriptions\Traits\HasGracePeriodUsage;
use Logicalcrow\Subscriptions\Traits\HasPricing;
use Logicalcrow\Subscriptions\Traits\HasSubscriptionPeriodUsage;
use Logicalcrow\Subscriptions\Traits\HasTrialPeriodUsage;
use Logicalcrow\Subscriptions\Traits\HasSchedules;
use Carbon\Carbon;
...
class PlanSubscription extends Model
{
    ...
    public function __construct(array $attributes = [])
    {
        ...
        $this->setTable(config('logicalcrow.tables.plan_subscriptions'));
    }
    ...
    public function getRules(): array
    {
        return [
            'tag' => [
                ...
                'alpha_dash',
                'max:150',
                Rule::unique(config('logicalcrow.tables.plan_subscriptions'))->where(function ($query) {
    ...
    {
        return $this->hasMany(config('logicalcrow.models.plan_subscription_feature'), 'plan_subscription_id', 'id');
    }
    ...
    public function usage(): \Illuminate\Database\Eloquent\Relations\HasManyThrough
    {
        return $this->hasManyThrough(
            config('logicalcrow.models.plan_subscription_usage'),
            config('logicalcrow.models.plan_subscription_feature'),
    ...
    {
        if (!$ignoreFallback && config('logicalcrow.fallback_plan_tag')) { // Do not cancel if a fallback plan is set
            $plan = Plan::getByTag(config('logicalcrow.fallback_plan_tag'));
            if (!$plan) {
                throw new UnexpectedValueException('Fallback plan ' . config('logicalcrow.fallback_plan_tag') . ' does not exist.');
            }
     ...
     * @param int $uses
     *
     * @return \Logicalcrow\Subscriptions\Models\PlanSubscriptionUsage|null
     */
